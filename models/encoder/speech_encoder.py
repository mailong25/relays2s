# https://github.com/VITA-MLLM/Freeze-Omni/blob/main/models/encoder/encoder.py
import sys
import time
import numpy as np
import torch
...
from models.masks import make_pad_mask
...
class SpeechEncoder(torch.nn.Module):
    def __init__(
        ...
        if global_cmvn is None:
            self.global_cmvn  = GlobalCMVN(torch.randn(encoder_input_dim),
                                           torch.randn(encoder_input_dim))
        else:
    ...
    def forward(self, xs, ilens, decoding_chunk_size=None, num_decoding_left_chunks=None):
        ...
        # padding left for causal CNN
        pad_size = 3
        xs = F.pad(xs, (0, 0, pad_size, 0))
        ilens = ilens + pad_size
        
        assert(len(xs.shape)) == 3
        if self.global_cmvn is not None:
            xs = self.global_cmvn(xs)
        
        assert xs.size(1) == int(ilens.max().item())
